    ...
    recipe.builder === 'shaped'
      ...
      : recipe.builder === 'round'
        ...
        : recipe.builder === 'sphere'
          ...
          : recipe.builder === 'knit'
            ? buildKnit(recipe.rows, W, yr, recipe.knitFace ?? (recipe.knitFlip ? 'garter' : 'stockinette'), recipe.knitGaugeScale ?? 1, recipe.knitCourseScale ?? 1, recipe.knitStitch, recipe.knitCables, recipe.knitPurlCols ? new Set(recipe.knitPurlCols) : undefined)
            : buildContinuous(rows, W, yr, { stitchAt: recipe.pattern, noTurn: recipe.noTurn, gaugeYr: recipe.gaugeYr, postReliefScale: recipe.postReliefScale })
