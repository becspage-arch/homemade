...
| **mrdisc** (magic ring + rounds) | flat amigurumi circle: MR anchor, 6 sc in ring, +6/round continuous no-turn spiral, polar frame + radial blocked pull + table + fasten-off (§8c) | ◑ **audit-clean (126/126), LOOK still NOT at the bar.** 2026-07-10: added `legReliefScale` 0.7 (calms same-face leg bulge; 0.6 strands an inner hook) — moved it from knotted roving to concentric-but-lumpy rounds, a real step but Rebecca confirmed NOT customer grade (random lumps throughout). Then tried a tangential ANGULAR HOLD in the relax to order the rounds — it made clumping WORSE (gap-CV 1.1→2.2) and broke a hook, because the lumps are inner-round CROWDING (too many stitches for the small-radius circumference — measured, gap-CV ≈1.1 at baseline), not stitches sliding out of slots; pinning the angle just locks the crowding in. Reverted. **Root cause = crowding + same-face relief; the relief/ordering lever family (crownLay×2, leg-relief, angular-hold) is exhausted and none reach the bar.** 2026-07-11: measured the disc's INNER rounds are collision-SATURATED (91% jammed at the 1.25yr floor vs flat sc's uniform ~70%), then tried both construction-tier levers (2 attempts each) — BOTH break the interlock: soft collision (§9 chain precedent, 1.0/1.1yr) weakens the hold → hooks same-side; radial pitch (drift 1.2/1.35) stretches the hook's reach to its below-crown → strands. The wall: the hook interlock needs FIRM collision + TIGHT radial spacing to hold, but that firm+tight combo is exactly what jams the crowd — decrowding by either lever unlinks it. NEXT needs an interlock/stitch-geometry redesign for rounds (a hook that tolerates decrowding), NOT a parameter — every parameter family (crownLay, leg-relief, angular-hold, soft-collision, radial-pitch) is now exhausted. legReliefScale 0.7 kept as the best-so-far WIP base. Do NOT present as done. |
...
| **knitrib** (1×1 rib, KNIT) | per-COLUMN pull side (even cols +z, odd −z), constant up each column — a purl column is a knit column seen from behind (§8d) | ✅ **LOCKED 2026-07-11** (Rebecca; Fable cross-checked the theory). Audit 240/240; columns settle cleanly alternating (+0.35..0.39 knit / −0.34..−0.41 purl). Built on `buildKnit`'s per-(course,column) `faceSign`; stockinette + garter bit-identical. Final density: `knitGaugeScale 0.85` packs the columns so the knit-V ribs read crisp + vertical with the purl valleys tight/recessed (was too exposed). |
...
| **garter** (KNIT) | stockinette loop + worked-face flip per course + per-course CORRUGATION (`bz = 0.7yr·fz`, gated — stockinette + rib untouched) | ◔ audit-clean (240/240) — the corrugation restores a full collision diameter between each leg crossing and the head it passes. **DENSITY fixed, TEXTURE not (2026-07-11).** Density pass (`knitCourseScale 0.7` + `knitGaugeScale 0.85`) closed the see-through gaps → solid dense fabric. But Rebecca: it reads as UPRIGHT loop clusters, not garter's horizontal purl-bump CATERPILLAR ridges. Probed the settled geometry: the corrugation forces the LEGS to swing ±2.6yr (dominant → the upright clusters) while the heads sit ±0.5yr (weak) — real garter is the inverse. Traced the tension: the corrugation clearance NEEDS big legs + small heads (leg(j+1) at −1.8 must clear head(j) at −0.3), so boosting the head eats the interlock; boosting the SINKERS instead (audit-clean) still rendered as upright clusters (big legs dominate). Same interlock-vs-look wall as mrdisc — needs a construction/interlock redesign, not a parameter. Density scales are the kept baseline; do not present. |
...
| **ball** (3D SPHERE) | full amigurumi ball on the curved-surface machinery (§8c-3D) | ◔ **audit-clean (320/320)** — the first 3D object: MR pole anchor, 6-in-ring, canonical ±6 staggered rounds, equator 30, mirrored decs, fasten-off into the bottom pole. Rendered 2026-07-10 (gate 0.774): true spherical silhouette, sits on the table, and the LOWER hemisphere reads as shingled crochet rounds — but the top cap is a jumble of fat loops (the disc's same-face-rounds knottiness, worst at the crowded pole). Look iteration = the SAME open item as mrdisc; the geometry is proven. Rebecca confirmed NOT passing 2026-07-10. |
