...
| **postrib** (1×1 fp/bp rib) | alternating fpdc/bpdc columns → raised ribs beside recessed valleys | ✅ **LOCKED 2026-07-07** (Rebecca delegated). Reads clearly as post-stitch ribbing vs a clean 1×1 rib ref (doradoes). **Caveat noted at lock:** ours is more OPEN than a tightly-packed real rib (columns show daylight; the reference's touch) — a looser gauge, still recognisably ribbing. Basketweave = same move in blocks, still `wip` (needs a reference + build). |
