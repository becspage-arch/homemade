...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPublicMetadata({
    title: `${cfg.title} stitch guide: symbols, abbreviations and how to work them`,
...
export default async function StitchGuidePage({ params }: PageProps) {
  ...
  // Crochet and knitting split UK / US terminology and need the toggle.
  // Cross-stitch does not: a full cross is a full cross either side of the
  // Atlantic, so the toggle and the explanation of it are both dropped.
  const hasTerminologySplit = groups.some((g) =>
    g.stitches.some((s) => s.ukName && s.usName && s.ukName !== s.usName),
  )
    ...
    <div id="stitch-reference-root" className="stitches-page" data-terminology="uk">
      ...
      <header className="stitches-hero">
        ...
        <p className="stitches-lede">
          Every stitch in one place: what each chart symbol means, its name and abbreviation,
          and a plain reminder of how it is worked. Keep it open beside a pattern, or print it
          and save it as a PDF for the pattern folder.
        </p>
      </header>

      <StitchReferenceControls
        craft={craftSlug}
        initial="uk"
        showTerminologyToggle={hasTerminologySplit}
      />

      <section className="stitches-howto no-print" aria-label="How to use this guide">
        {hasTerminologySplit ? (
          <p>
            A {cfg.title.toLowerCase()} chart draws the piece as a map of symbols, one per stitch.
            Match a symbol on your chart to the key below to see which stitch it is. The{' '}
            <span className="stitches-term-uk">UK</span>
            <span className="stitches-term-us">US</span> toggle switches every name and
            abbreviation between British and American terminology. Worth knowing that UK{' '}
            <em>double crochet</em> and US <em>single crochet</em> are the same stitch under two
            names.
          </p>
        ) : (
          <p>
            A cross-stitch chart is a grid, one square per stitch. Each square carries a symbol
            or a block of colour, and the key beside the chart says which floss that symbol
            stands for. Match a symbol to the list below to see which stitch it asks for and how
            it is worked. The names are the same in Britain and America, so there is no
            terminology to switch.
          </p>
        )}
      </section>
                ...
                <li key={s.slug} className="stitch-row">
                  <div className="stitch-symbol-cell" aria-hidden="true">
                    {s.chartSymbol ? (
                      ...
                      <span className="stitch-symbol-none" title="Worked per pattern, with no single chart symbol">
                        —
                      </span>
                    )}
                  </div>
      ...
      <footer className="stitches-footer no-print">
        <p>
          ...
          <Link href="/make">Studio</Link> to follow it stitch by stitch. This guide is always a
          tap away from inside it.
        </p>
      </footer>
    </div>
  )
