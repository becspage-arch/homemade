...
export interface SewingPatternData {
  ...
  // ──────────────────────────────────────────────────────────────────────
  // Freesewing wrapper integration (S-5a + S-5b + S-5c).
  //
  // When `isFreesewingDesign` is true, the pattern body is generated by
  // the freesewing engine at apps/web/src/lib/sewing/grading/. The S-1
  // `pieces[]` array stays empty in that case; the viewer + projector +
  // print pipeline read the SVG below instead.
  //
  // `freesewingShowcaseSvg` is rendered server-side at CYC default
  // measurements so anonymous browse works without a client-side draft
  // call. Signed-in personalisation lands in S-5d.
  // ──────────────────────────────────────────────────────────────────────
  isFreesewingDesign: boolean
  freesewingDesignSlug: string | null
  freesewingShowcaseSvg: string | null
  freesewingShowcaseCacheKey: string | null
}
