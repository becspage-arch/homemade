...
/**
 * Faceted filter controls for the food-category recipe library
 * (phase_dish_type_001). Deliberately mirrors the cross-stitch
 * PatternLibraryGrid sidebar so selection works the same across the site:
 * a left rail of grouped, individually-toggleable filter buttons (click an
 * active one to clear it), a "Clear all", a result count and a sort select.
 * The results grid itself is server-rendered and passed in as `children`, so
 * this client wrapper never needs the recipe data.
 *
 * Replaces the earlier horizontal chip strips, which ran off-screen and gave
 * no obvious way to deselect.
 */
...
interface RecipeLibraryControlsProps {
  groups: RecipeFacetGroup[]
  /** current value per group key (null = not selected). */
...
const SORT_OPTIONS: { value: string; label: string }[] = [
  { value: 'familiar', label: 'Familiar first' },
  { value: 'popular', label: 'Most loved' },
  { value: 'newest', label: 'Newest' },
  { value: 'quick', label: 'Quickest' },
]

export function RecipeLibraryControls({
  groups,
  current,
  count,
  sort,
  ...
  const update = (key: string, value: string | null) => {
    ...
    router.push(qs ? `${basePath}?${qs}#recipes` : `${basePath}#recipes`, { scroll: false })
  }
