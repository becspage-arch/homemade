...
interface KnittingPatternCardData {
  id: string
  slug: string | null
  name: string
  /** The card image: the photoreal hero when the pattern has one, else the
   *  saved chart thumbnail. There is no loom hero column on KnittingPattern
   *  yet (crochet's `loomHeroMediaId` equivalent), so this is a two-step
   *  fallback, not crochet's three-step one. */
  imageMediaId: string | null
  difficulty: 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED' | null
  finishedSizeText: string | null
  yarnWeightLabel: string | null
  needleSizeLabel: string | null
  premium: boolean
...
interface Props {
  patterns: KnittingPatternCardData[]
...
/**
 * Knitting pattern library grid. Cloned from `CrochetPatternGrid` — same
 * shell, sidebar filters, inline search and cross-craft tag facets, with
 * knitting-specific card content (yarn weight / needle size / finished
 * size). Cards open straight into the Knitting Studio.
 */
export function KnittingPatternGrid({
  patterns,
  subCategories,
  tagFacets,
  designerFacets,
      ...
      <section className="cross-stitch-library-main">
        ...
        {patterns.length === 0 ? (
          ...
          <ul className="cross-stitch-library-grid">
            {patterns.map((p) => (
              <KnittingCard key={p.id} pattern={p} />
            ))}
          </ul>
        )}
      </section>
...
function KnittingCard({ pattern: p }: { pattern: KnittingPatternCardData }) {
  const href = p.slug
    ? `/studio/knitting?knittingPatternSlug=${encodeURIComponent(p.slug)}`
    : `/studio/knitting?knittingPatternId=${encodeURIComponent(p.id)}`
  const meta = [
    p.subCategoryName,
    p.difficulty ? pretty(p.difficulty) : null,
    p.finishedSizeText,
  ].filter(Boolean)
  const yarn = [p.yarnWeightLabel, p.needleSizeLabel].filter(Boolean).join(' · ')
  return (
