 ...
 * container on AWS Fargate — same pinned Blender, same `loom_render.py`, same
 * scene.json + samples. This module is the caller side: it hands the scene to
 ...
 * long-running service) and we poll it to completion. The AWS CLI is used
...
function aws(args: string[], json = false): unknown {
  const res = spawnSync('aws', args, { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 })
  if (res.error) {
    throw new Error(
      `Could not run the AWS CLI ('aws ${args[0]} ${args[1]}'): ${res.error.message}. ` +
        'The Fargate render path needs the AWS CLI v2 on PATH.',
    )
  }
  if (res.status !== 0) {
    const tail = `${res.stdout ?? ''}\n${res.stderr ?? ''}`.trim().slice(-800)
    throw new Error(`aws ${args[0]} ${args[1]} failed (exit ${res.status}):\n${tail}`)
  }
...
export async function fargateRenderBase(
  ...
  // 1. scene.json -> S3
  aws(['s3', 'cp', resolve(scenePath), `s3://${cfg.bucket}/${sceneKey}`, '--content-type', 'application/json', ...r])
  ...
  aws(['s3', 'cp', `s3://${cfg.bucket}/${outKey}`, resolve(basePath), ...r])
}
