...
- Cross-stitch Studio v1 (Phase B, 2026-06-08) — schema migration (Pattern, Designer, PatternLicense, UserPatternProgress, Tutorial.patternId, User.studioPreferences/paperSize/defaultFabricCount/defaultBrand/defaultStrands; PatternType, LicenseType, Visibility, PaperSize, FlossBrand enums; legacy User.paperSize String? upgraded to enum). One canonical chart engine (SVG, pan/zoom, X-shape stitches with deterministic jitter, layered colour buckets, back-stitch + French knots, low-zoom fallback). Studio app at /studio/cross-stitch with empty / loaded / new-blank / new-from-photo states, overlay palette + floss-key drawers, autosave (silent fork on first edit of a library pattern), Zustand store, undo/redo command pattern, IndexedDB local-only store for logged-out users. Photo-to-chart route with live preview (sharp + image-q Wu quantiser + curated DMC table + perceptual-distance nearest-floss + confetti minimisation). PDF export (cover + floss key + tiled chart pages with overlap + per-page tile-position thumbnail map + B&W toggle, A4 / Letter / Legal / A3). Floss-list text export. Public library at /cross-stitch/patterns with sub-cat / difficulty / size / colour-count / backstitch / french-knot filters and featured / newest / popular / size sort. Pattern detail page at /cross-stitch/patterns/[slug]. Tutorial integration — patternInset TipTap node + scripts/migrate-cross-stitch-charts.ts one-shot to convert every legacy crossStitchChart node to a canonical Pattern row + patternInset reference. Premium gating wrappers (config flag, all free at v1). 3 hand-crafted seed patterns ready (scripts/seed-cross-stitch-patterns.ts: tiny-alphabet-sampler, garden-bird-silhouette, wild-flower) attributed to the Stitching Mama designer stub. Stitching Mama catalogue import waits for Phase C.
