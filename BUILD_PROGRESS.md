...
Fix scripts shipped alongside JSON: `k6-fix-tools.mjs` (invalid tool slugs + fall→autumn), `k6-fix-voice.mjs` (grade-level rewrites, banned phrases, unused glossary, safety-block conversion), `k6-batch-upload.sh` (idempotent voice-check + upload loop for all three dirs).
