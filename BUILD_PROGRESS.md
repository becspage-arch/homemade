...
switched on. The crochet Inngest lane is being converted to a session-driven
routine on that rule.
...
**Per-TYPE rules**, one file per type in `packages/db/scripts/qc-makeability-rules/` (`shared.ts` helpers + generic checks, `index.ts` = `auditMakeability()` dispatcher keyed on (type, category, subcategory), `loader.ts` resolves linked craft-pattern rows + chart body-nodes into real chart facts). Files: pattern-cross-stitch / -crochet / -knitting / -needlework-counted / -needlework-surface / -project, recipe, technique, stitch, practice, growing-guide, remedy, herb-profile, reading. Binary block / skip, no warning tier, AI-only.

**Numbers (hard).** Audited all **9,035 PUBLISHED** rows → **240 un-published (2.7%)**, **8,795 kept** (8,328 fully clean + 467 makeable-but-carry-a-voice-nit, kept + flagged). Per type: TECHNIQUE 136, PATTERN 48, GROWING_GUIDE 32, HERB_PROFILE 10, RECIPE 9, STITCH 2, READING 2, PRACTICE 1, REMEDY 0. **Worst categories by % failure:** cross-stitch 42.6% (23/54), garden 25.8% (32/124), needlework 13.7% (26/190), sustainability 11.5% (70/608), herbal-medicine 7.0% (10/142). Top blocking reasons: no step-by-step (130, mostly decision/comparison guides mis-typed as TECHNIQUE), counted-needlework no chart (25), growing guide no common-problems section (22), **cross-stitch no chart (20 — all 20 cross-stitch PATTERN rows, as predicted)**, no designer attribution (19), no sowing depth/spacing (10), recipe no quantified ingredients (9), placeholder (9), herb no habitat (8).

**Surprise findings.** (1) All 20 cross-stitch PATTERN tutorials had **no chart anywhere** — confirmed un-publishable, every one. (2) Needlework charts live in a body `patternInset` node → `Pattern.data`, not on NeedleworkPattern; **all 20 needlepoint patterns have no chart**, plus 5 of 42 blackwork. (3) Several recipes (baked-croissants, chocolate-and-chestnut-yule-log, mince-pie-cookies…) carry ingredientsList items with **null amounts** — ingredients listed with no quantities. (4) A large slice of sustainability/animals "TECHNIQUE" rows are decision/comparison/reference content (ev-charger-decision-guide, hot-compost-vs-warm-compost, bantam-coop specs) with no procedure — mis-typed, held for re-type. (5) Herb profiles are medicinal-shaped (actions/uses/doses/safety) and omit the botanical sections (habitat, identification) the type needs.

**Kept, not nuked (locked don't-over-prune).** Voice nits (em/en dash, banned phrasing) and improvement flags (open-ended practice duration, missing completion criterion, missing materials/cut-list/clay-body/firing on project patterns) are **flagged, not un-published** — as hard rules they removed makeable content (a wired light switch, a repair task with nothing to cut, a journal prompt). Calibration took 7 dry-run passes; the raw first pass falsely failed 63.6% (glossary `termId`-vs-`termSlug` false positive, trailing-`\b` regex bugs, string ingredient amounts, heading text bleeding into action-verb detection) — all caught and fixed before any DB write.

**Un-published** the 240: status → DRAFT, structured `qcBlockReason` set (`gate: 'makeability'`), **`publishedAt` + slug preserved**, nothing deleted. Verified post-apply: 8,795 PUBLISHED, 240 DRAFT carrying the gate reason, publishedAt preserved.

**Wired into the publish path** via new `qc-publish-gate.ts` (`checkRowPublishable` = completeness AND makeability), called from `gatedPublishDrafts` (batch flip) and `uploadTutorial` (autopilot upload, post-write pull-back). **Gate test passed:** a chart-less cross-stitch PATTERN through `gatedPublishDrafts` was BLOCKED + held DRAFT; a charted control published. Autopilot cannot bypass.

**Image work untouched** — no `heroMediaId` changes, no regeneration. Documented only: 27 PUBLISHED rows have null heroMediaId, 2 of the 240 un-published do (separate fresh-eyes image session). **Autopilot stays OFF** (Rebecca's call after rebuild). Artefacts: `packages/db/docs/makeability-audit-2026-06-16.json` (per-slug) + `makeability-summary-2026-06-16.md`. No deploy expected (script + DB-data changes; admin UI publish path unchanged — wiring makeability there needs the rules promoted to a shared package export, flagged as follow-up).
...
- Cooking 1,068 (anchors + pilot-10 + personal-recipe ingest + bulks 001–040; note: QC blocks since bulk-039 reduced published count from 1,295). Baking 1,012 (pilot-10 + bulks 001–029 + bulk-004b). Mindset 1,001 (bulks 001–025 + editorial pass — TARGET MET). Herbal-medicine 143 (bulk-001 + bulk-002 + bulk-003 + bulk-004). Fibre arts 556 (bulk-001 through bulk-015). Wood-natural-craft 352 (bulk-001 through bulk-010). Paper & word 546 (bulk-001 through bulk-014). Animals & smallholding 563 (bulk-001 through bulk-018). Home & repair 585 (bulk-001 through bulk-015). Natural home 476 (bulk-001 through bulk-013). Sustainability 608 (bulk-001 through bulk-015). Pottery & ceramics 440 (bulk-001 through bulk-010). **Garden 124** (permaculture batch-01 + vegetables batch-01 + microgreens batch-01). **Needlework 190** (foundations/surface-embroidery/blackwork bulk-001 ×150 + needlepoint bulk-001 ×40). **Crochet 357** (Foundations 57 + Stitches 60 + Motifs 120 + bulk-004 TECHNIQUE ×40 + bulk-005 HOMEWARE ×40 + bulk-006 PATTERN shawls/wraps ×40). **Knitting 244** (K-6 ×150 + K-4.4 ×14 + mitt-glove bulk-001 ×40 + shawl-wrap bulk-002 ×40). The other 3 categories are private placeholders until each crosses 10 published rows.
...
- Autopilot — baking bulk-029 (autopilot-queue-extra, 2026-06-19): 40 PUBLISHED (→ 1,012) — biscuits ×6 (lebkuchen, pfeffernusse, polvorones-spanish, shrewsbury-biscuits, stroopwafels, viennese-whirls), bread ×7 (hokkaido-milk-bread, malted-brown-loaf, pain-de-campagne, seeded-spelt-tin-loaf, tiger-bread-dutch-crunch, vollkornbrot-dense-rye, yemenite-kubaneh), cake-decorating ×3 TECHNIQUE (cake-stencilling-technique, fault-line-cake-technique, geode-cake-technique), cakes ×8 (almond-and-cherry-cake, coffee-walnut-traybake, dundee-cake, earl-grey-tea-cake, hazelnut-meringue-layer-cake, marmalade-loaf-cake, pear-and-almond-cake, tres-leches-cake), pastries ×6 (fregolata, kouign-amann, pain-au-chocolat, paris-brest, saint-honore-gateau, sfogliatelle-ricotta), pies ×5 (leek-and-bacon-tart, pecan-pie, steak-and-ale-pie, summer-berry-tart-creme-patissiere, treacle-tart), scones ×4 (oat-and-pecan-scones, sourdough-discard-scones, spelt-and-honey-scones, stilton-and-walnut-scones), sweets-confectionery ×1 (honeycomb-cinder-toffee). RECIPE ×37, TECHNIQUE ×3. Pipeline fixes: 11 files used `content[ingredient]` format instead of `attrs.items[]` (converted); 22+ files had invalid ingredient slugs (corrections: egg→eggs, salt→salt-table, fine-sea-salt→sea-salt-fine, dried-yeast→yeast-dried, strong-white-bread-flour→strong-bread-flour, flaked-almonds→almonds-flaked, orange-zest→orange, lemon-zest→lemon, carrots→carrot, onions→onion, pears→pear, instant-coffee→coffee-instant, beef-stock→stock-beef, ale→beer, cinnamon→cinnamon-ground, candied-peel→mixed-peel, gelatine→gelatine-leaves, wafer-circles removed); 2 files grade-level fix (paris-brest pastry cream step, saint-honore-gateau intro paragraph); paris-brest "fall"→"drop". Multi-section ingredientsList fix for saint-honore-gateau + sfogliatelle-ricotta. Hero fill: 24 unsplash, 14 pexels. QC: 40/40 PASS. Report: docs/baking-bulk-029-report.md.

- Autopilot — baking bulk-028 (autopilot-queue, 2026-06-19): 40 PUBLISHED (→ 990) — bread ×8 (conchas-mexican, flour-tortillas-homemade, injera-ethiopian, kartoffelbrot-german, kulich-russian, lavash-armenian, mantou-steamed-plain, paratha-plain-griddle), cake-decorating ×8 (buttercream-basket-weave, chocolate-curls-shavings, chocolate-leaf-transfers, korean-flower-piping, naked-cake-technique, sugar-paste-rose, swiss-meringue-buttercream, tiered-cake-assembly), cakes ×8 (battenberg-cake, bolo-de-mel-madeira, charlotte-royale, dacquoise-hazelnut-disc, gateau-chocolat-pruneaux, quatre-quarts-french-pound, tarta-de-santiago, zuccotto-dome-cake), pastries ×6 (beigli-hungarian-walnut, colomba-di-pasqua, fleurons-puff-pastry, fraisier-patisserie, kouignettes-individual, tarte-tropezienne), pies ×4 (blackcurrant-plate-pie, flan-parisien, gypsy-tart-kentish, quiche-lorraine), sweets-confectionery ×4 (acid-drops-boiled-sweets, marzipan-fruits-moulded, peanut-butter-cups-dark, toffee-apples), scones ×2 (cheese-and-marmite-scones, sweet-potato-scones). RECIPE ×32, TECHNIQUE ×8. Pipeline fixes: ingredientsList items required `ingredientSlug` + `amount` field names (not `slug` + `quantity`); `eggs` plural slug for whole eggs; `sugar-thermometer` correct tool slug (not `candy-thermometer`); three TECHNIQUE files needed MISTAKES_RE `avoid` signal; battenberg-cake step 2 had 3 consecutive short imperatives in prose (prose-prep-steps fix: merged sentences); chocolate-curls-shavings had all technique sections as paragraphs with no orderedList (body-missing-method fix: converted to orderedList); naked-cake-technique grade-level fix; swiss-meringue-buttercream glossary tooltip coverage fix. QC: 40/40 PASS. Report: docs/baking-bulk-028-report.md.
...
- Autopilot — pottery-ceramics bulk-010 (autopilot-queue, 2026-06-15): 40 PUBLISHED (400 → 440) — hand-building-no-equipment ×19 PATTERN (coil-built-snail-sculpture-air-dry, slab-built-arch-bookends-air-dry, pinch-pot-heart-shaped-dish, slab-built-monstera-leaf-wall-art, paper-clay-wreath-wall-decoration, polymer-clay-faux-malachite-pendant, polymer-clay-sculpted-bear-figurine, coil-built-narrow-bottle-vase-air-dry, pinch-pot-mushroom-cluster-figurine, paper-clay-forest-cabin-ornament, polymer-clay-faux-blue-goldstone-pendant, slab-built-cat-shaped-planter-air-dry, polymer-clay-ocean-drop-earrings, air-dry-clay-ring-holder-hand-shape, slab-built-hexagonal-planter-set-air-dry, paper-clay-mushroom-cluster-sculpture, polymer-clay-faux-jasper-stone-pendant, polymer-clay-butterfly-hair-comb, polymer-clay-gradient-ocean-wave-clip) + slab-built-divided-trinket-dish-air-dry; surface-decoration ×4 TECHNIQUE (gold-leaf-application-air-dry-clay, photo-transfer-onto-air-dry-clay, cold-wax-medium-finish-air-dry-clay, mica-powder-burnish-polymer-clay); clay-fundamentals ×3 TECHNIQUE (making-a-plaster-slump-mould-air-dry, testing-clay-porosity-and-sealing, conditioning-polymer-clay-smooth-finish); throwing ×7 PATTERN (throwing-a-stoneware-dinner-plate, throwing-a-large-casserole-lid-and-knob, throwing-a-set-of-cereal-bowls, throwing-a-flared-serving-bowl-stoneware, throwing-a-yunomi-stoneware, making-custom-texture-stamps-from-clay, throwing-a-stoneware-planter-with-drainage-hole); glazing/decoration ×4 TECHNIQUE (mocha-diffusion-glaze-decoration, exterior-glaze-pouring-technique, neriage-layered-clay-colouring, combed-slip-decoration-on-greenware); firing ×3 TECHNIQUE (raku-post-firing-reduction-chamber, electric-kiln-candling-preheat, cone-10-reduction-firing-schedule). Voice fixes: grade-level fixes across 11 files total (7 troubleshooter/listItem paths in first pass; 4 paragraph-level rewrites in second pass after qc-fix surfaced stricter threshold violations); 4 re-uploads to sync DB. Hero fill: 40/40 pexels via fixup-hero-fill.ts. QC: 40/40 processed, 40 pass, 0 still_blocked (two passes — first cleared 36/40, second cleared remaining 4 after paragraph rewrites + re-uploads). Note: session ran across two context windows; 33 files published in first run, 7 grade-level failures fixed and published in second run; final 4 QC paragraph rewrites done in the resumed context window. Pottery & ceramics 400 → 440.
...
- Autopilot — garden microgreens batch-01 (autopilot-queue, 2026-06-14): 40 PUBLISHED (84 → 124) — garden/microgreens sub-category. GROWING_GUIDE ×40. Growing guides ×19 (broccoli, radish, kale, beetroot, cabbage, chard, kohl-rabi, leek, mustard, onion, pea, rocket, chive, dill, coriander, fennel, basil, buckwheat, sunflower), sowing guides ×11 (broccoli, radish, kale, beetroot, chard, pea, leek, basil, coriander, buckwheat, sunflower), harvesting guides ×5 (broccoli, kale, radish, pea, sunflower), variety-selection guides ×5 (radish, kale, pea, sunflower, herb-microgreens). Covers brassica cotyledons (broccoli/radish/kale/cabbage/chard/kohl-rabi/mustard/rocket), shoots (pea/sunflower/buckwheat), allium microgreens (chive/leek/onion), and herb microgreens (basil/coriander/dill/fennel). All BEGINNER difficulty; garden.subTopic = growing / sowing / harvesting / variety-selection; all 12-month plantingMonths + harvestMonths (indoor year-round crops); containerFriendly + indoorFriendly = true. Voice fixes: clinical vocab (mucilage/mucilaginous → "gel coating" in sowing-basil-microgreens), em-dash replacements across 15+ files, grade-level rewrites across 25+ files (voice-check threshold 12.0 + QC threshold 11.0 both cleared). Hero fill: 39 unsplash, 1 pexels, 0 failed. QC: 40 microgreens 0 blocked (5 slugs required two rounds of grade-level fix + re-upload + hero-refill + qc-fix --include-unfixable). Garden 84 → 124.
...
- Autopilot — cooking bulk-051 (autopilot-queue, 2026-06-20): 40 PUBLISHED (1,458 → 1,498) — Middle Eastern theme: dips/spreads ×5 (musabaha, labneh-zaatar, labneh-balls-olive-oil, mast-o-musir, kuku-sibzamini), flatbreads/pastry ×9 (manakish-cheese, manakish-lahm-bi-ajeen, naan-e-barbari, naan-e-sangak, naan-e-taftoon, pide-peynirli, pide-sucuklu, su-boregi, sfiha), grills/kebabs ×6 (shish-kebab, urfa-kebabi, kebab-barg, kebab-soltani, kofte-cig, falafel-wrap), Persian rice ×4 (chelow, sabzi-polo, lubia-polo, albaloo-polo), stews/stuffed ×8 (khoresh-gheymeh, khoresh-karafs, khoresh-e-mast, dolma-biber, dolma-yaprak, stuffed-courgettes-koosa, stuffed-cabbage-rolls-malfouf, imam-bayildi), soups/porridge ×3 (aash-e-anar, ash-e-jow, halim), kibbeh ×2 (kibbeh-bil-sanieh, kibbeh-nayyeh), eggs ×2 (menemen, sucuklu-yumurta), lentil/grain ×1 (mujaddara-bulgur). RECIPE ×40. Voice-check fixes: servings-yield conflicts (16 files — servings→null for discrete-item yields: labneh-balls-olive-oil, falafel-wrap, dolma-biber, dolma-yaprak, kofte-cig, manakish-lahm-bi-ajeen, naan-e-barbari, naan-e-sangak, sfiha, stuffed-cabbage-rolls-malfouf, stuffed-courgettes-koosa, urfa-kebabi; yieldDescription→null for portion-count: manakish-cheese, naan-e-taftoon, pide-peynirli, pide-sucuklu); grade-level rewrites (19 files — "Where this dish lives" and some method steps simplified to grade ≤8); banned phrase "essentially" removed (kebab-barg); raw-hours fix "72 hours"→"2 to 3 days" (labneh-balls-olive-oil); ingredient slug fixes (6 wrong slugs: sugar-caster→caster-sugar, allspice-ground→allspice, butter-unsalted→unsalted-butter, cherry→cherries, dried-mint→mint-dried, paprika→paprika-sweet — fixed in both ingredientsList fields and body scaling tokens via sed). Heroes: 28 Unsplash + 21 Pexels, 0 failed. QC: 85 candidates, 76 pass, 9 still_blocked. Briefs: docs/bulk-batch-051-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-050 (autopilot-queue, 2026-06-20): 40 PUBLISHED (1,418 → 1,458) — pressure-cooker theme: stocks/broths ×3 (pressure-cooker-beef-stock, pressure-cooker-bone-broth, pressure-cooker-pho-style-broth), risottos ×3 (pressure-cooker-chicken-risotto, pressure-cooker-mushroom-risotto, pressure-cooker-leek-risotto), braises/stews ×6 (pressure-cooker-lamb-shanks, pressure-cooker-beef-bourguignon, pressure-cooker-chilli-con-carne, pressure-cooker-beef-brisket, pressure-cooker-pork-ribs, pressure-cooker-gammon), curries ×3 (pressure-cooker-chicken-curry, pressure-cooker-butter-chicken, pressure-cooker-chicken-tikka-masala), chicken ×2 (pressure-cooker-shredded-chicken, pressure-cooker-whole-chicken), beans/legumes ×3 (pressure-cooker-dried-beans, pressure-cooker-chickpeas-from-dried, pressure-cooker-black-beans), eggs/dairy ×2 (pressure-cooker-hard-boiled-eggs, pressure-cooker-yoghurt), grains/oats ×6 (pressure-cooker-oatmeal, pressure-cooker-steel-cut-oats, pressure-cooker-rice, pressure-cooker-brown-rice, pressure-cooker-quinoa, pressure-cooker-pearl-barley), vegetables ×3 (pressure-cooker-mashed-potato, pressure-cooker-beetroot, pressure-cooker-artichokes), soups ×7 (pressure-cooker-chicken-soup, pressure-cooker-minestrone, pressure-cooker-french-onion-soup, pressure-cooker-split-pea-soup, pressure-cooker-squash-soup, pressure-cooker-lasagna-soup, pressure-cooker-congee), fish/desserts ×2 (pressure-cooker-poached-salmon, pressure-cooker-cheesecake). RECIPE ×40. Voice-check fixes: em-dash batch replace (40 files — Node.js); grade-level rewrites (20+ files); servings-yield conflicts (9 files — servings null, yieldDescription kept); unused glossary terms removed (7 files: soffritto, deglaze, braise×2, marinade, incubation, caramelise); tool slug fixes (two-forks→removed, potato-ricer→removed, thermometer→removed, springform-tin-18cm→springform-tin, electric-hand-whisk→hand-mixer); ingredient slug fixes (leeks→leek, tomato-ketchup→ketchup, soy-sauce→soy-sauce-light×3, lasagna-sheets→lasagne-sheets, digestive-biscuits→digestive-biscuit). Heroes: 0 new fills. QC: 80 processed, 80 pass, 0 still_blocked. Briefs: docs/cooking-bulk-050-briefs/. Report: docs/cooking-bulk-050-report.md. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-049 (autopilot-queue-extra, 2026-06-20): 40 PUBLISHED (1,378 → 1,418) — air-fryer theme: chicken/poultry ×10 (air-fryer-whole-chicken, air-fryer-spatchcock-chicken, air-fryer-salt-and-pepper-wings, air-fryer-lemon-pepper-wings, air-fryer-honey-garlic-wings, air-fryer-korean-style-wings, air-fryer-chicken-parmesan, air-fryer-chicken-katsu, air-fryer-chicken-tikka, air-fryer-tandoori-chicken), beef/pork/lamb ×8 (air-fryer-rump-steak, air-fryer-fillet-steak, air-fryer-kofta, air-fryer-lamb-chops, air-fryer-kebab-skewers, air-fryer-chorizo, air-fryer-cumberland-sausages, air-fryer-roast-beef), fish/seafood ×6 (air-fryer-sea-bass, air-fryer-mackerel-fillets, air-fryer-calamari-rings, air-fryer-scallops, air-fryer-crab-cakes, air-fryer-breaded-prawns), vegetarian/sides ×11 (air-fryer-roasted-peppers, air-fryer-butternut-squash, air-fryer-kale-chips, air-fryer-halloumi-fries, air-fryer-paneer-cubes, air-fryer-beetroot, air-fryer-potato-rosti, air-fryer-roasted-tomatoes, air-fryer-cheese-toastie, air-fryer-hard-boiled-eggs, air-fryer-breakfast-sandwich), meal builds/other ×5 (air-fryer-salmon-teriyaki, air-fryer-mozzarella-sticks, air-fryer-chicken-fajitas, air-fryer-chicken-caesar-wrap, air-fryer-roast-vegetable-traybake). RECIPE ×40. Voice-check fixes: en-dash bulk replace (40 files — `–` → `-` in time ranges); em-dash bulk replace (40 files — `—` → `, ` throughout body); grade-level rewrites (entries 09 + 10 — long marinate ingredient-listing sentences split into shorter declaratives); slug fixes: butter→unsalted-butter (entries 22, 33, 35), mustard-dijon→dijon-mustard (entry 23), romaine-lettuce→cos-lettuce (entry 39); unit fixes: mayonnaise ml→tbsp (entry 23), caesar-dressing tbsp→ml (entry 39). Heroes: 42 entries processed (38 Unsplash + 4 Pexels, 0 failed). QC: 73 processed, 73 pass, 0 still_blocked. Briefs: docs/cooking-bulk-049-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-048b (autopilot-queue, 2026-06-20): 33 net PUBLISHED (1,345 → 1,378) — Tex-Mex: tacos ×7 (beef-tacos, soft-beef-tacos, carnitas-tacos, chicken-tacos, fish-tacos, shrimp-tacos, pulled-pork-tacos), burritos ×4 (beef-burrito, chicken-burrito, bean-and-rice-burrito, wet-burrito), enchiladas ×5 (beef-enchiladas, cheese-enchiladas, chicken-enchiladas-red, chicken-enchiladas-verde, enchiladas-suizas), nachos ×3 (nachos-supreme, beef-nachos, vegetarian-nachos), quesadillas ×4 (quesadilla-cheese, quesadilla-chicken, quesadilla-steak, quesadilla-vegetable), fajitas ×4 (fajitas-beef, fajitas-chicken, fajitas-prawn, fajitas-vegetable), soups ×2 (tortilla-soup, taco-soup), dips and salsas ×6 (guacamole, pico-de-gallo, salsa-roja, salsa-verde, seven-layer-dip, queso-dip), sides and foundational ×5 (refried-beans, mexican-rice, loaded-fries, chilli-cheese-fries, chimichanga). RECIPE ×40. Voice-check fixes: 25 BLOCKED on first pass; grade-level rewrites across 16 "Where this dish lives" paragraphs (historical sentences scoring grade 12+ simplified to grade 8 or below); em-dash replacements across 14+ files; negation pattern in salsa-roja ("not just darkened colour" → "actual blackened patches"); banned phrase "genuinely" → "properly" (chilli-cheese-fries); Americanism "shrimp" → "prawns" (fajitas-prawn body); wrong tool slug in fish-tacos (instant-read-thermometer → thermometer-probe); prose-prep-steps manual fix for 2 files (chilli-cheese-fries assembly step merged, vegetarian-nachos bean step merged). No tomatillo or tortilla-chips slugs in master table: salsa-verde uses green peppers + jalapeños (sourceType SYNTHESISED), nachos use corn-tortilla with prepNote. No beef-skirt slug: fajitas-beef uses beef-ribeye. Ingredient slug used for fries: potato with prepNote "cut into 1 cm sticks". Heroes: 75 filled by fixup-hero-fill.ts (0 failed). QC: 33 processed, 33 pass, 0 still_blocked. Briefs: docs/bulk-batch-048-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-047 (autopilot-queue-extra, 2026-06-20): 77 uploaded, 40 net PUBLISHED (1,265 → 1,305) — British pub plates ×12 (bangers-and-mash, cottage-pie, shepherds-pie, welsh-rarebit, prawn-cocktail, liver-and-onions, macaroni-cheese, fish-and-chips, sausage-casserole, fish-finger-sandwich, beans-on-toast, bubble-and-squeak), British stews ×8 (lancashire-hotpot, beef-and-guinness-stew, scotch-broth, cullen-skink, irish-stew, beef-stew-with-dumplings, welsh-cawl, chicken-casserole-cider-tarragon), Anglo-Indian curry house ×20 (chicken-tikka-masala, chicken-korma, lamb-rogan-josh, tarka-dhal, chana-masala, saag-aloo, onion-bhaji, pilau-rice, raita, chicken-jalfrezi, tandoori-chicken, lamb-bhuna, kedgeree, saag-paneer, aloo-gobi, bombay-potato, chicken-tikka, garlic-naan, chicken-dopiaza, vegetable-samosa), British classics ×37 (atholl-brose, bacon-sandwich, black-pudding-breakfast, boiled-bacon-and-cabbage, boxty, bread-and-butter-pudding, chip-butty, chip-shop-curry-sauce, chicken-goujons, coronation-chicken, coronation-chicken-sandwich, cucumber-sandwich, devilled-eggs, devilled-kidneys, devilled-mackerel, egg-and-chips, eggy-bread, ham-and-mustard-sandwich, jacket-potato-cheese-and-beans, jacket-potato-tuna-mayo, oxtail-soup, pie-and-mash, potted-shrimps, sausage-sandwich, scampi, scotch-woodcock, singin-hinnies, smoked-haddock-chowder, smoked-salmon-sandwich, soda-farls, spaghetti-bolognese, spotted-dick, stovies, toad-in-the-hole, treacle-tart, tuna-pasta-bake, welsh-cakes). Fixes: season enum UPPERCASE (10 files: autumn/winter→AUTUMN/WINTER); null-amount unit fixes (59 files: sea-salt-fine g→pinch, black-pepper tsp→grind, nutmeg tsp→grind, chives/parsley g→handful, malt-vinegar ml→splash, ketchup ml→splash, paprika/cayenne tsp→pinch, sunflower-oil for frying→500 ml); tool slug fixes (4 files: casserole-dish→dutch-oven, thermometer-instant-read→instant-read-thermometer, tart-tin-23cm→tart-tin, round-cutter→biscuit-cutters); ingredient slug fixes (58 files: ground-coriander→coriander-ground, chicken-thighs→chicken-thigh, flat-leaf-parsley→parsley-flat, fresh-coriander→coriander-fresh, yogurt-natural→plain-yoghurt, lamb-neck-chops→lamb-neck, pork-sausages→sausages-pork, cabbage→cabbage-white, and ~35 others); prose-prep-steps manual splits (4 files: aloo-gobi, boxty, chicken-dopiaza, macaroni-cheese — packed listItem paragraphs split into separate list items). Heroes: 77 filled (72 Pexels, 5 flux-schnell). QC: 0 still_blocked. Cooking 1,265 → 1,305. Briefs: docs/bulk-batch-047-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-046 (autopilot-queue-extra, 2026-06-20): 40 uploaded, 18 net PUBLISHED (1,247 → 1,265) — French cuisine classics: beef braises ×3 (boeuf-bourguignon, coq-au-vin, pot-au-feu), roast chicken ×5 (poulet-roti-classique, poulet-basquaise, poulet-a-la-moutarde, poulet-a-l-estragon, poulet-chasseur), duck & cassoulet ×3 (confit-de-canard, cassoulet-toulousain, daube-de-boeuf-provencale), veal & lamb braises ×5 (blanquette-de-veau, navarin-d-agneau, veau-marengo, gigot-d-agneau, petits-pois-a-la-francaise), fish & seafood ×5 (sole-meuniere, moules-marinieres, bouillabaisse, brandade-de-morue, coquilles-saint-jacques), soups ×4 (soupe-a-l-oignon, vichyssoise, soupe-au-pistou, and others), vegetables ×5 (gratin-dauphinois, ratatouille, carottes-vichy, tian-de-legumes, and others), eggs & charcuterie ×4 (oeufs-en-cocotte, quiche-lorraine, salade-nicoise, pate-de-campagne), steak & mains ×2 (steak-au-poivre, gigot-d-agneau-a-la-broche), desserts ×7 (pot-de-creme-au-chocolat, iles-flottantes, creme-caramel, crepes-suzette, tarte-tatin, mousse-au-chocolat, clafoutis). 22 were upserts to records already published in bulk-036/037. New tools seeded (1): gratin-dish (flat-spatula→fish-slice, oven-safe-soup-bowls→ramekins, blender→blender-jug mapped to existing slugs). New ingredients seeded (7): potatoes-floury, new-potatoes, veal-shoulder, pearl-onions, sirloin-steak, grand-marnier, monkfish. Voice-check fixes: grade-level rewrites (3+ files), banned phrase "genuinely" (2 files), em-dash in tarte-tatin prepNote. Slug fixes: ~15 ingredient + tool slug remaps (dark-chocolate-70→dark-chocolate, electric-mixer→hand-mixer, eating-apples→apple-eating, bacon-lardons→lardons, etc.). Heroes: 36 filled (14 Unsplash, 21 Pexels, 1 flux-schnell). QC (continuation session): 6 prose-prep-steps issues fixed (coquilles, creme-caramel, quiche-lorraine, soupe-a-l-oignon, soupe-au-pistou, vichyssoise — packed list-item paragraphs split into separate list items) → 0 still_blocked. Cooking 1,247 → 1,265. Briefs: docs/bulk-batch-046-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-045 (autopilot-queue-extra, 2026-06-20): 40 uploaded, 36 net PUBLISHED (1,211 → 1,247) — Desserts: British hot puddings ×12 (jam-roly-poly, marmalade-pudding, christmas-pudding, sussex-pond-pudding, clootie-dumpling, brioche-bread-and-butter-pudding, croissant-pudding, stovetop-rice-pudding, tapioca-pudding, semolina-pudding, lemon-delicious-pudding, vegan-crumble), cobblers and poached fruit ×7 (apple-cobbler, peach-cobbler, berry-cobbler, pears-in-red-wine, cardamom-poached-pears, poached-white-peaches, poached-rhubarb-in-elderflower), trifles ×6 (trifle, sherry-trifle, strawberry-trifle, black-forest-trifle, lemon-trifle, christmas-trifle-with-mince-pies), panna cotta and posset ×4 (panna-cotta-coffee, panna-cotta-lemon, vegan-panna-cotta, posset-lime), mousses ×4 (lemon-mousse, mango-mousse, raspberry-mousse, coffee-mousse), ice cream flavours ×6 (strawberry-ice-cream, mint-choc-chip-ice-cream, salted-caramel-ice-cream, coffee-ice-cream, pistachio-ice-cream, rum-and-raisin-ice-cream), ice cream treats ×1 (knickerbocker-glory). RECIPE ×40. Voice-check fixes: em-dash batch replace all 40 files in body text nodes and troubleshooter attrs; em-dash in metadata fields (yieldDescription "Serves 6–8" → "Serves 6 to 8", ingredientsList prepNotes, batchNotes, makeAheadNotes — 9 files fixed after upload held them at DRAFT); grade-level rewrites (11 files — opening paragraphs for ice creams, posset, semolina storage para, sherry-trifle method, vegan panna cotta intro, raspberry-mousse and salted-caramel-ice-cream troubleshooter entries); banned phrase "genuinely" removed (christmas-pudding troubleshooter); Americanism "fall" → "autumn" (marmalade-pudding); string-array glossaryTerms cleared to [] (18 files from first-agent draft). New ingredients seeded (3): sponge-fingers, tapioca-pearls, brioche-loaf. Slug fixes: salt removed from ingredientsList in 4 files (not in master slug table); null nutmeg amount fixed in semolina-pudding (set to 0.25 tsp). Heroes: 39 filled (26 Unsplash, 13 Pexels, 0 failed). QC: 79 processed, 79 pass, 0 still_blocked. Cooking 1,211 → 1,247. Briefs: docs/bulk-batch-045-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-044 (autopilot-queue, 2026-06-20): 40 uploaded, 40 net PUBLISHED (1,171 → 1,211) — Salads: american-style ×4 (caesar-salad-chicken, vegan-caesar-salad, cobb-salad, chefs-salad), coleslaw ×4 (coleslaw-southern-fried-style, coleslaw-asian-sesame, coleslaw-red-cabbage-apple, vegan-coleslaw), potato salads ×3 (potato-and-egg-salad, potato-salad-german-style, new-potato-salad-chive), tuna-bean ×1 (tuna-and-bean-salad), pasta salads ×3 (pasta-salad-pesto, pasta-salad-tomato-mozzarella, pasta-salad-chicken-sweetcorn), grain salads ×6 (couscous-salad-roasted-vegetables, couscous-salad-feta-olive, bulgur-and-herb-salad, rice-salad-classic, wild-rice-salad-cranberry, quinoa-roasted-vegetable-salad), legume salads ×3 (lentil-and-goats-cheese-salad, lentil-and-mushroom-salad, three-bean-salad), roasted vegetable ×4 (roast-vegetable-salad-feta, roast-pepper-salad-anchovy, roast-cauliflower-salad-tahini, roast-aubergine-salad-pomegranate), garden salads ×5 (beetroot-orange-watercress-salad, carrot-orange-salad-moroccan, peach-and-prosciutto-salad, tomato-and-onion-salad, smashed-cucumber-salad), slaws and fennel ×4 (fennel-orange-olive-salad, fennel-and-celery-slaw, slaw-broccoli-bacon, slaw-kale-apple), summer salads ×3 (mozzarella-and-peach-salad, chicken-and-bacon-salad, chicken-and-mango-salad). RECIPE ×40. Voice-check fixes: grade-level rewrites ("Where this dish lives" paragraphs — systematic simplification across 22 files, all exceeding grade 12.0 threshold); banned phrase "genuinely" (files 12, 16, 25, 32); Americanism "fall" false positive (file 10 — "fall apart" → "break apart"); "pomegranate molasses" Americanism warnings (file 28 — 6 occurrences, non-blocking, accepted as standard British ingredient name). QC: 82 of 85 auto-fixed; 3 still_blocked (pre-existing crochet hero-missing issues unrelated to this batch). Cooking 1,171 → 1,211. Briefs: docs/bulk-batch-044-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-043 (autopilot-queue, 2026-06-20): 40 uploaded, 32 net PUBLISHED (1,139 → 1,171) — Preserves: jams ×10 (strawberry-jam, raspberry-jam, blackberry-jam, gooseberry-jam, plum-jam, rhubarb-and-ginger-jam, apricot-jam, damson-jam, fig-jam, blueberry-jam), marmalades ×5 (seville-orange-marmalade, blood-orange-marmalade, three-fruit-marmalade, lemon-marmalade, ginger-marmalade), fruit curds ×5 (lemon-curd, raspberry-curd, passion-fruit-curd, rhubarb-curd, orange-curd), chutneys ×7 (mango-chutney, apple-chutney, tomato-chutney, red-onion-chutney, caramelised-onion-chutney, plum-chutney, beetroot-chutney), pickles ×5 (piccalilli, pickled-red-onions, pickled-red-cabbage, pickled-beetroot, pickled-cucumbers-quick), ferments ×3 (sauerkraut, kimchi, lacto-fermented-carrots), cordials ×5 (elderflower-cordial, lemon-cordial, blackcurrant-cordial, raspberry-cordial, rhubarb-cordial). RECIPE ×40. Voice-check fixes: em-dash / en-dash replacement (all 40 files); grade-level rewrites (files 01, 02, 11, 15, 34 — long compound sentences split into short declaratives); glossary setting-point removed from glossaryTerms where term only appeared in headings not paragraph text (files 03–05, 08–10); glossaryTooltip marks injected inline where setting-point mentioned with 104/105 °C temperature (files 01, 02, 06, 07, 11–15). Slug corrections (15 remaps, 21 files): blackcurrant→blackcurrants, chinese-cabbage→napa-cabbage, fresh-thyme→thyme-fresh, fresh-dill→dill-fresh, salt→salt-table, ground-cumin→cumin-ground, ground-coriander→coriander-ground, ground-turmeric→turmeric-ground, ground-cinnamon→cinnamon-ground, ground-allspice→allspice, ground-ginger→ginger-ground, black-peppercorns→peppercorns-black, tomatoes→tomato, plum→plums, carrots→carrot. New ingredients seeded (2): elderflower, mooli. New tools seeded (4): glass-bottle, glass-jar-500ml, glass-jar-750ml, glass-jar-1l. Heroes: 45 filled (34 Unsplash, 11 Pexels). QC: 80 processed, 42 pass, 38 still_blocked (all hero-missing from earlier baking batches — none from this batch). Cooking 1,139 → 1,171. Briefs: docs/bulk-batch-043-briefs/. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-042 (autopilot-queue-extra, 2026-06-19): 40 uploaded, 40 net PUBLISHED (1,099 → 1,139) — Breakfasts: cooked breakfast plates ×4 (full-scottish-breakfast, full-irish-breakfast, full-welsh-breakfast, vegan-full-english), egg techniques ×9 (poached-eggs, soft-boiled-eggs, boiled-eggs-with-soldiers, jammy-boiled-eggs, hard-boiled-eggs, coddled-eggs, fried-egg, scrambled-eggs-smoked-salmon, omelette-plain), egg dishes ×5 (omelette-ham-and-cheese, omelette-mushroom, omelette-smoked-salmon, kippers-with-butter, smoked-haddock-poached-egg), porridge and oat breakfasts ×5 (porridge-with-apple-cinnamon, porridge-with-peanut-butter, overnight-oats-peanut-butter, overnight-oats-berries, baked-oats), muesli and granola ×3 (muesli, maple-pecan-granola, vegan-granola), chia and yoghurt ×4 (chia-pudding, yoghurt-with-honey-and-walnuts, yoghurt-parfait, berry-compote), fruit compotes ×2 (apple-compote, berry-compote), smoothies ×2 (smoothie-green, smoothie-berry), pancakes and pastries ×4 (british-thin-pancakes, sourdough-pancakes, liege-waffles, pain-perdu), enriched breakfasts ×2 (croissant-french-toast, crumpets-with-butter), bagel ×1 (bagel-with-lox). Voice-check fixes: em-dashes (all 40 files — `—` replaced throughout body); en-dashes (15 files — numeric ranges `8–10` → `8 to 10`); banned phrase "genuinely" (6 files — files 04, 19, 20, 24, 38, 39); banned phrase "essentially" (file 24); banned phrase "honest" (file 03); historical-figure gloss ×2 (files 10, 13 — "the Victorian cookery writer" before "Mrs Beeton"); servings-yield conflict ×7 (files 05, 06, 08, 09, 34, 35, 36 — nulled yieldDescription); Americanism "fall" ×2 (files 31, 37 — "fall apart" → "break apart"); brand trademark Nutella ×1 (file 34 — → "chocolate hazelnut spread"); tricolon ×1 (file 33 — berry list rewritten); grade-level rewrites (22+ files — opening and method paragraphs simplified); banned phrase "ideal for" ×1 (file 11 — "a 20 cm pan is ideal for" → "a 20 cm pan is the right size for"); body structure fix (files 11–40 had ingredientsList at top level not inside body.content — transformed by _fix-bulk042-body-structure.ts); orderedList method steps added (files 06, 07, 10, 19 — completeness gate required an orderedList node; added 3-step summary before detailed method sections). All 40 pass voice-check before upload; all 40 PUBLISHED (initial upload gave 35 PUBLISHED + 5 DRAFT held by completeness gate: fried-egg "ideal for" ban + 4 files missing orderedList; all 5 fixed and re-uploaded). Heroes: no hero fill this session (image-relevance queue written to docs/image-relevance-queue-cooking-bulk-042.json). QC: all 40 PUBLISHED. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-041 (autopilot-queue, 2026-06-19): 40 uploaded, 31 net PUBLISHED (1,068 → 1,099) — British pies ×9 (steak-and-ale-pie, steak-and-kidney-pie, steak-and-kidney-pudding, cottage-pie, shepherds-pie, fish-pie, chicken-and-mushroom-pie, chicken-and-leek-pie, chicken-and-ham-pie), fish pies ×2 (smoked-haddock-and-leek-pie, salmon-and-dill-pie), vegetarian pies ×2 (vegetarian-shepherds-pie, vegan-cottage-pie), pasties and rolls ×4 (cornish-pasty, cheese-and-onion-pasty, beef-and-stilton-pasty, sausage-roll), pub classics ×7 (beer-battered-haddock, bangers-and-mash, liver-and-onions, welsh-rarebit, toad-in-the-hole, stargazy-pie, kedgeree), potted dishes and starters ×4 (potted-shrimps, devilled-kidneys-on-toast, potted-beef, cullen-skink), pork ×3 (pork-chops-with-cider-and-apples, pork-chops-mustard-cream, pork-belly-crackling), soups and broths ×6 (scotch-broth, lancashire-hotpot, london-particular, cock-a-leekie, mulligatawny, irish-stew), braises and stews ×3 (braised-oxtail, beef-stew-with-dumplings, beef-olives). Voice-check: BOM fix required (PowerShell Set-Content added UTF-8 BOM to all 40 files — stripped after discovery); global em-dash replace (34 files — `—` replaced throughout body, sourceNotes, troubleshooter, infoPanel; en-dash `–` in "26–28 cm" range also caught); grade-level rewrites (9 files: fish-pie, cornish-pasty, sausage-roll, welsh-rarebit, liver-and-onions, irish-stew, mulligatawny, stargazy-pie, vegetarian-shepherds-pie); glossary-coverage (15 files — added glossaryTooltip marks for: deglaze/blind-bake, braise ×2, suet, poach, tarragon, crimp, dill, batter, roux, kedgeree, pearl-barley, crackling, pilchard, clarified-butter); servings-yield conflict ×7 (cornish-pasty, cheese-and-onion-pasty, sausage-roll, welsh-rarebit, potted-shrimps, beef-and-stilton-pasty, potted-beef — nulled servings, kept yieldDescription); banned phrase "genuinely" ×2 (mulligatawny, stargazy-pie); brand trademark Greggs ×1 (sausage-roll — genericised to "A national bakery chain"); historical-figure gloss ×1 (steak-and-kidney-pudding — added "the Victorian cookery writer" before Mrs Beeton). Slug corrections: tools frying-pan→frying-pan-26 (×4), large-frying-pan→frying-pan-30 (×1), butchers-twine→kitchen-twine (×1); ingredients sardines→sardines-fresh, split-peas→split-peas-green, beef-topside→beef-rump. Heroes: 8 Unsplash, 26 Pexels, 1 flux-schnell, 0 failed. QC: 70 processed, 64 pass, 6 still_blocked. This fire ran on Claude Sonnet 4.6.
- Autopilot — cooking bulk-040 (autopilot-queue, 2026-06-19): 40 uploaded, 40 net PUBLISHED (1,028 → 1,068) — Italian pizza ×14 (pizza-dough-24h, pizza-margherita, pizza-marinara, pizza-diavola, pizza-quattro-stagioni, pizza-quattro-formaggi, pizza-prosciutto-e-funghi, pizza-salame-piccante, pizza-bianca, pizza-fritta, calzone-ricotta-e-salame, sfincione, pizza-al-taglio, pizza-in-teglia), antipasti & frittata ×10 (bruschetta-al-pomodoro, caprese-salad, frittata-di-zucchine, frittata-di-cipolle, frittata-di-asparagi, mozzarella-in-carrozza, carciofi-alla-romana, panzanella, caponata, frittata-di-pasta), soups/pulses/sides ×10 (parmigiana-di-melanzane, fagioli-alluccelletto, ribollita, minestrone, pasta-e-fagioli, pasta-e-ceci, pasta-e-lenticchie, polenta-morbida, finocchi-al-forno, spinaci-ripassati), seafood ×6 (cozze-alla-marinara, cozze-al-vino-bianco, vongole-in-bianco, calamari-fritti, tonno-alla-siciliana, insalata-di-polpo). Voice-check: grade-level rewrites (files 26, 35, 39), banned phrase "genuinely" (files 36, 40), unused glossary terms inline (files 32, 37, 40). Slug corrections: ~20 ingredient slugs, 8 tool slugs; 1 new ingredient seeded (pizza-dough-ball); sourceType HOMEMADE_ORIGINAL→CLASSIC (files 04, 08, 14). Heroes: 32 Unsplash, 8 Pexels, 0 failed. QC: 43 processed, 38 pass, 5 still_blocked (pre-existing: battenberg-cake, chocolate-curls-shavings, crochet-magic-ring, how-to-hold-a-crochet-hook, how-to-work-a-treble). Chain: n/a. This fire ran on Claude Sonnet 4.6. Report: docs/cooking-bulk-040-report.md.
...
- Autopilot — animals-smallholding bulk-018 + bulk-017 rollover (autopilot-queue, 2026-06-15): 80 uploaded, 117 net PUBLISHED (446 → 563) across two combined sets — **Set A (bulk-018, 40 files):** rabbits ×8 (angora-rabbit-wool-harvesting, rabbit-winter-care, handling-and-restraining-rabbits, treating-snuffles-in-rabbits, rabbit-nail-trimming, fostering-orphan-rabbit-kits, rabbit-colony-welfare-assessment, rabbit-dental-problems), pigs ×8 (pig-feed-ration-calculation, winter-housing-for-outdoor-pigs, pig-weigh-tape-estimating-liveweight, creep-feeding-piglets, troubleshooting-farrowing-problems, finishing-pigs-to-carcass-weight, making-air-dried-ham, eaml2-movement-paperwork), bees ×6 (spring-feeding-syrup-to-bees, combining-two-colonies-newspaper-method, introducing-a-mated-queen-in-a-cage, european-foulbrood-recognition, solar-wax-melter, removing-supers-after-harvest), poultry ×6 (raising-ducks-alongside-chickens, diagnosing-respiratory-illness-chickens, candling-eggs-during-incubation, treating-coccidiosis-in-chicks, duck-pool-setup, using-a-broody-bantam-to-hatch-eggs), sheep-and-goats ×6 (foot-rot-treatment-in-sheep, scanning-ewes-for-pregnancy, shearing-with-hand-shears, fitting-ear-tags-sheep, making-ricotta-from-goats-whey, cashmere-combing-from-goats), smallholding-skills ×6 (calculating-stocking-rates-for-grazing, building-a-hay-rack-from-timber, water-trough-cleaning-and-maintenance, setting-up-a-stock-trailer-for-safe-transport, reading-a-basic-soil-test-report, managing-a-smallholding-in-a-heatwave). **Set B (bulk-017 carryover, 40 files, written by prior context, uploaded this session):** bees ×7 (robbing-behaviour-and-prevention, supering-up-in-spring, heather-honey-harvest, moving-bees-for-orchard-pollination, collecting-pollen-with-a-trap, understanding-bee-nutrition-through-the-year, oxalic-acid-vaporisation-for-varroa), poultry ×7 (raising-meat-birds-for-the-table, preserving-eggs-in-water-glass, fitting-anti-peck-devices, treating-pasty-vent-in-chicks, managing-laying-hens-through-severe-winter, building-a-pallet-chicken-coop, making-a-simple-incubator-at-home), sheep-and-goats ×7 (recognising-and-treating-pinkeye-in-sheep, recording-lamb-weights-and-growth, neonatal-lamb-tube-feeding, understanding-fleece-grades, pine-and-cobalt-deficiency-in-sheep, managing-triplet-lambs, recognising-white-muscle-disease-in-lambs), rabbits ×6 (rabbit-heat-stroke-recognition-and-response, building-a-rabbit-morant-ark, rabbit-territorial-behaviour-and-group-management, rabbit-post-kindle-doe-checks, rabbit-cage-floor-options, rabbit-dental-problems), pigs ×7 (curing-a-whole-leg-ham-at-home, cold-smoking-cured-pork, pig-farrowing-litter-records, batch-farrowing-for-smallholders, understanding-pig-grading-from-the-abattoir, managing-weaner-pigs-in-cold-weather, managing-gilts-in-first-pregnancy), smallholding-skills ×6 (installing-solar-powered-electric-fencing, measuring-pasture-cover-with-a-plate-meter, building-a-mobile-strip-grazing-fence, smallholding-bookkeeping-basics, managing-shared-grazing-agreements, planning-a-multi-species-water-system, sourcing-affordable-bedding-materials). **New tools seeded (32):** antibiotic-hoof-spray, cashmere-comb-steel, dematting-comb, disposable-gloves-long, dressmakers-tape-measure, duck-water-trough, egg-candler-torch, exterior-wood-preservative, feed-scoop-3kg, hosepipe-connector, hutch-cover-weatherproof, large-non-reactive-container, meat-hook-stainless, pet-feeding-syringe-1ml, pig-creep-feeder, pig-weigh-tape, poultry-drinker-1l, poultry-feeder-hanging, rubber-feeding-trough-large, saline-solution-wash, shearing-stone, sheep-ear-tags-electronic, small-animal-grooming-brush, small-animal-nail-clippers, small-animal-shears, solar-wax-melter (tool), stiff-brush-trough, styptic-powder, towel-restraint, towels-birthing, water-bottle-rabbit, wheelbarrow. **Voice fixes:** em-dash bulk fix (excerpt + sourceNotes fields, 17 files), grade-level rewrites (32 paragraphs across 28 files), medical-claim "treats" (2 files), safety-block heading removal (2 files), glossary-coverage missing tooltip (2 files), raw-hours "72 hours→3 days" (1 file), projectSchedule field rename dayOffset/day/label/description → offsetDays/title/body + added stepNumber (2 files), TECHNIQUE→PATTERN type fix on schedule-bearing file (1 file). **Heroes:** 60 filled (55 pexels, 4 unsplash, 1 flux-schnell, 0 failed). **QC (08:59 run):** 91 processed, 87 pass, 4 still-blocked (crochet pre-existing, not this batch). Chain: 8/10. This fire ran on Claude Sonnet 4.6. Briefs: docs/animals-smallholding-bulk-018-briefs/.
...
- Autopilot — natural-home bulk-013 (autopilot-queue, 2026-06-14): 42 PUBLISHED (436 → 476) — soap ×10 (lard-olive, lemon-poppy-seed, carrot-turmeric, tallow-rose, sea-kelp-peppermint, pine-tar, neem-tea-tree-shampoo-bar, melt-and-pour-beginner, oat-milk, lavender-kaolin-clay), candles ×9 (soy-grapefruit-mint, beeswax-orange-peel-pillar, soy-tobacco-vanilla, coconut-wax-jasmine, soy-driftwood-sea-salt, beeswax-tealights, soy-pine-forest, coconut-wax-rose-geranium-votive, rapeseed-wax-lemon-thyme), beauty ×7 (arnica-bruise-balm, beeswax-lip-balm, rosehip-face-oil, peppermint-foot-balm, honey-oat-face-mask, shea-butter-body-lotion, rosemary-scalp-oil), cleaning ×7 (toilet-bowl-fizz-tablets, wood-floor-cleaner, stainless-steel-paste, natural-drain-unblocker, upholstery-spot-cleaner, natural-furniture-polish, carpet-deodoriser-powder), fragrance ×9 (beeswax-lavender-solid-perfume, bergamot-vetiver-room-spray, rosemary-eucalyptus-simmer-pot, lavender-rose-drawer-sachets, ylang-ylang-jasmine-room-spray, pine-juniper-room-spray, sweet-orange-clove-simmer-pot, vanilla-sandalwood-wax-melt, geranium-lime-linen-spray). Voice-check: 42/42 pass (0 errors, 34 clean, 8 warnings-only). Seeded 10 new ingredients (arnica-flowers-dried, vanilla-absolute, rapeseed-wax, sea-buckthorn-oil, perfumers-alcohol, jasmine-absolute, fragrance-oil-jasmine, fragrance-oil-pine-cedarwood, essential-oil-grapefruit, eucalyptus-leaves-dried) + 8 new tools (lip-balm-tubes, votive-candle-moulds, face-mask-brush, microwave-safe-jug, amber-glass-bottle-30ml, silicone-wax-melt-mould, microfibre-flat-mop, pump-bottle-200ml). Heroes: 1 unsplash, 36 pexels, 5 flux-schnell, 0 failed. QC: 78 processed, 74 pass, 4 still-blocked. Ran on Claude Sonnet 4.6. See docs/natural-home-bulk-013-report.md.
...
| 16 | Natural home | 800 | 476 PUBLISHED (bulk-001 through bulk-013 + 2 anchors, 2026-05-20 → 2026-06-14) | ✅ **bulk-013 (autopilot-queue, 2026-06-14):** 42 uploaded (436 → 476) — soap ×10, candles ×9, beauty ×7, cleaning ×7, fragrance ×9. Seeded 10 new ingredients + 8 new tools. Heroes: 36 pexels, 1 unsplash, 5 flux. Voice-check: 42/42 pass. QC: 78 processed, 74 pass, 4 still_blocked. Report: docs/natural-home-bulk-013-report.md. **bulk-012 (autopilot-queue, 2026-06-12):** 40 uploaded, 32 net new (404 → 436) — soap ×8 (goats-milk-honey, hot-process, hemp-seed, liquid-castile, shampoo-bar-basic, salt-bar, coffee-scrub, cocoa-butter-vanilla), candles ×8 (coconut-wax-coconut-lime, soy-geranium-ylang, beeswax-birthday, soy-amber-oud, soy-lavender-vanilla-layered, travel-tin, soy-eucalyptus-spearmint, soy-fig-cassis), beauty ×12 (dry-shampoo, hot-oil-hair, exfoliating-foot-cream, bay-rum-aftershave, lavender-sleep-balm, glycerin-rosewater-mist, neem-scalp-serum, jojoba-cleansing-balm, herbal-facial-steam, sea-salt-hair-spray, muscle-soak-salts, calendula-infused-oil), cleaning ×6 (copper-paste, dishwasher-tablet, mould-mildew-spray, oven-paste, glass-cleaner, laundry-powder), fragrance ×6 (rose-geranium-room-spray, frankincense-myrrh-diffuser, lemon-verbena-sachets, floral-spring-simmer-pot, citrus-herb-linen-spray, cedar-rose-sachets). Voice-check: 40/40 pass (0 errors, 22 clean, 18 warnings-only). Seeded 21 new ingredients + 3 new tools. QC: 38 auto-fixed, 36 still-blocked (hourly routine). Resume of a prior interrupted session — briefs were pre-authored; this fire completed the upload cycle. Report: docs/natural-home-bulk-012-report.md. **bulk-011 (autopilot-queue, 2026-06-02):** 40 net PUBLISHED (367 → 402). **bulk-010 (2026-06-01):** 36 net PUBLISHED (331 → 367). Prior: bulk-001–009 + 2 anchors. | 0.55 |
| 17 | Sustainability | 700 | 608 PUBLISHED (bulk-001 through bulk-015, 2026-05-20 → 2026-06-14) | ✅ **bulk-015 (autopilot-queue-extra, 2026-06-14):** 40 PUBLISHED (568 → 608) — insulation-and-draughtproofing ×10, solar-and-energy ×8, composting ×8, water ×6, waste-reduction ×5, off-grid ×3. Voice-check: 40/40 pass. Heroes: 28 unsplash, 11 pexels, 1 flux. QC: 78 processed, 62 pass, 16 still-blocked (grade-level-strict at qc threshold 11). **bulk-014 (autopilot-queue-extra, 2026-06-12):** 40 PUBLISHED (529 → 568) — insulation-and-draughtproofing ×10, solar-and-energy ×8, composting ×6, foraging ×16 (sub-cat live). bulk-013 (2026-06-02): 36 net PUBLISHED — insulation ×10, solar ×8, composting ×8, water ×6, waste-reduction ×5, off-grid ×3. Prior: bulk-001–012. Reports: bulk-001–005, bulk-008–013. | 0.87 |
| 18 | Cross-stitch | 230 | 80 PUBLISHED. **bulk-002 (autopilot-queue-extra, 2026-06-20):** 25 published (5 TECHNIQUE stitch-care tutorials + 20 project tutorials retyped PATTERN→TECHNIQUE: animals ×5, florals ×5, quotes-and-sayings ×5, pride-and-inclusive ×5). Bugs fixed: `textilePatternRule()` checking categorySlug in COUNTED_NEEDLEWORK; TECHNIQUE makeability gate completion-criterion phrases; upload path resolution. 73 → 80. **bulk-003 (autopilot-queue, 2026-06-20):** 40 briefs uploaded, all PUBLISHED — TECHNIQUE tutorials covering core skills, materials, finishing, display. 23 → 73. Report: docs/cross-stitch-bulk-003-report.md. | ✅ pipeline-setup READY (2026-06-08). targetTutorialCount=230. Autopilot round-robin ON for TECHNIQUE. | 0.23 |
| | **Total** | **31,930** | **1,686** | ~12 wks setup outstanding (new categories) + ~3 wks Cooking sub-extensions | ~28 weeks fill |
...
## Autopilot — Baking bulk-031 ✅ landed 2026-06-19

40 PUBLISHED. Resumed 40 briefs from a failed previous fire (17 uploaded in session 1; 23 uploaded in session 2 after fixes). Sub-categories: cake-decorating ×6, cakes ×9, sweets-confectionery ×8, pies ×6, biscuits ×6, bread ×5. Fixes applied across 23 re-uploaded files: glossaryTerms plain-string→object format (7 files: 6 tart + barbari); ingredient slug fixes strong-white-flour→strong-bread-flour, yeast-instant→yeast-fast-action, salt→sea-salt-fine (5 bread files), earl-grey-tea→tea-bag (earl-grey-truffles); grade-level rewrites (crystallised-ginger-homemade simmer step, gum-paste-peony equipment paragraph + outer-petals step); servings/yieldDescription conflict (9 cake files); sourceType DOCUMENTED→SYNTHESISED (barbari-iranian-flatbread); criticalTechniques mismatch cleared (gooseberry-and-elderflower-tart); cold-water→water slug (courgette-and-goat-cheese-tart). Voice-check 40/40 pass. Heroes: 3 new fills (Pexels, Unsplash, flux-schnell). QC: 68 processed, pass=68, still_blocked=0. Baking 1,048 → 1,088 (91% of 1,200 target).

## Autopilot — Cooking bulk-051 ✅ landed 2026-06-20

40 PUBLISHED Middle Eastern recipes. Sub-categories: Persian (naan-e barbari, halim, kuku sibzamini, ash-e jow, aash-e anar, kashk-e bademjan), Levantine (kibbeh bil sanieh, koosa mahshi, mansaf, sfiha, manakish cheese, shish tawook, fatteh, fattoush, labneh balls), Egyptian (kushari, molokhia), pan-Middle-Eastern (mujaddara with bulgur, falafel wrap, shakshuka with feta, musakhan, and others). Fixes applied across uploads: slug corrections (potato-floury→potato, eggs→eggs, spinach-baby→spinach, bulgur-fine→bulgur-wheat, whole-wheat-grain→bulgur-wheat, strong-white-bread-flour→strong-bread-flour, yeast-instant→yeast-fast-action, water-warm→water, pomegranate-juice→pomegranate, allspice-ground→allspice, bay-leaf→bay-leaves, flatbread→pitta-bread, lentils naming, macaroni→pasta-dried, red-cabbage→cabbage-red, paprika→paprika-sweet, green/brown-lentils slug format, bulgur-coarse→bulgur-wheat); tool slug corrections (baking-sheet→baking-tray, deep-frying-pan→deep-fryer, metal-skewers→skewers, glass-jar-sterilised→maceration-jar); completeness gate (10 files needed H2 "Method" heading added; 1 had an en-dash in a prepNote); grade-level rewrites on 4 entries; tricolon warnings cleared on 5 entries. Voice-check: 40/40 pass. Heroes: 20 new fills (3 Unsplash, 17 Pexels, 0 failed). QC: 68 auto-fixed, 0 still-blocked. Cooking 1,458 → 1,498.
