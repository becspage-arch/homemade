...
- Cooking 536 (anchors + pilot-10 + personal-recipe ingest + bulks 001–010). Baking 208 (pilot-10 + bulks 001–004). Mindset 24 (bulks 001–002 — 19 PRACTICE + 5 type-intro READING). The other 14 categories are private placeholders until each crosses 10 published rows.
...
| 2 | Baking | 3,000 | 208 PUBLISHED (10 pilot + 50 bulk-001 + 50 bulk-002 + 49 bulk-003 + 50 bulk-004, 2026-05-17) + 4 DRAFT anchor | ✅ schema + taxonomy + authoring prompt v2 + anti-tells + pilot-10 + bulk-001 + bulk-002 + bulk-003 + bulk-004 all landed. **bulk-004 (parallel-burner session, 2026-05-17):** 50 PUBLISHED covering all 7 active sub-categories: pies (7) + pastries (8) + biscuits (10) + cakes (10) + bread (7) + scones (3) + sweets-confectionery (5). 17 BEGINNER / 31 INTERMEDIATE / 2 ADVANCED. Voice-check: 11 em-dash pairs in sourceNotes + 3 body fixes + 1 medical-claim watchword + 1 banned phrase fixed on first run; 0 new anti-tells patterns. Report: `docs/baking-bulk-004-report.md`. Cumulative sub-category fill: bread 41, cakes 43, pies 26, pastries 28, biscuits 33, scones 18, sweets-confectionery 18, cake-decorating 5. Four anchor DRAFTs pending Rebecca review. Baking-specific TipTap blocks (baker's percentages, lamination schedule, sugar-stage panel) still ahead. **bulk-003 (parallel-burner session, 2026-05-17):** 49 PUBLISHED rotating bread (21) + cakes (20) + scones (3) + biscuits (5) back in after bulk-002's pies/pastries focus. 21 BEGINNER / 24 INTERMEDIATE / 4 ADVANCED. 13 voice-check failures on first run (all em-dash pairs in excerpt/sourceNotes/body) fixed and re-uploaded; 1 ingredient slug miss (lemon-juice not in master table) resolved by removing from no-bake-vanilla-cheesecake. Report: `docs/baking-bulk-003-report.md`. | 3 |
| 3 | Garden | 4,000 | 0 | Not started — ~1 wk setup | 4 |
| 4 | Herbal medicine | 2,500 | 0 | Not started — ~1 wk setup | 2.5 |
| 5 | Mindset | 4,300 | 24 PUBLISHED (bulks 001–002) + 11 DRAFT (anchor batch pending) | ✅ schema + backlog + authoring prompt v3 + anti-tells (24 entries) + 11 sub-categories + 11-entry anchor batch ready. **bulk-001 (autopilot, recovered 2026-05-17):** 20 briefs uploaded as PUBLISHED on the recovery fire (migration `20260619000000_phase_categories_targets_001` had since applied during cooking + baking autopilot runs). Breakdown: 15 PRACTICE rows across all 10 active practice types (TAPPING x2, ENERGY_STATEMENT x1, AFFIRMATION x2, SPELL x1, RITUAL x2, ACTIVITY x2, JOURNAL_PROMPT x2, VISUALISATION x1, MEDITATION x1, EMBODIMENT x1) + 5 type-intro READING rows (how-affirmations-work, how-spells-work, activities-as-practice, how-visualisations-work, how-embodiment-works). Sub-category fill is even across all 11 sub-categories. Mindset crosses the 10-row public threshold and is now `Public: ✓`. **bulk-002 (autopilot-queue, same-session recovery 2026-05-17):** 4 PRACTICE rows landed PUBLISHED after a halt-and-recover for the pending pottery migration — AFFIRMATION x2 (`steady-steady-steady` MONEY-v2/D4, `i-am-safe-even-when-the-number-is-small` MONEY-v2/D2), ENERGY_STATEMENT (`there-is-enough-now` MONEY-v2/D2 + Money-Zone/Ch2-5), RITUAL (`the-hand-on-heart-money-breath` MONEY-Journal + [PD]). Slice held to 4 for the Opus-model concession (scheduled-tasks runner still firing Opus, not the Sonnet its frontmatter requests). Report: `docs/mindset-bulk-002-report.md`. From bulk-003 onward the autopilot continues weighting under-represented life categories (Time, Friendship, Creativity, Big-picture identity, Grief, Trauma, Sexuality, Spiritual depth). Migration `20260614000000_phase_8_step_13_mindset_schema` ships PRACTICE / READING TutorialType values + 11-value `PracticeType` + 20-value `PracticeTarget` + `TimeBand` + `BestTime` + `PlanTier` + `PlanStatus` + `PlanSlotSource` enums + Tutorial practice-metadata columns + the six user-side tables. `docs/mindset-author.md` v3 + `docs/mindset-anti-tells.md` (24 entries, ~20 [block]) drive Mindset drafting. Voice-check Mindset extension (Anchor / price-mention / treats false-positives), plan generator, admin/public UI — still ahead. | 4.3 |
