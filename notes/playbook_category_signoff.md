...
The reusable, growing checklist for taking a category through sign-off. The strategy/lock rationale lives in [[playbook_category_signoff]]; THIS file is the operational how-to. When a new category appears (or a new one is invented), copy this, adapt Step 0, and write its worker prompt from it. Add every learning to the log at the bottom so each category gets easier.
...
- Attribute to the Homemade house designer via `ensureHouseDesigner()` from `@homemade/db`, never a hardcoded upsert (see [[reference_house_designer_canonical]]). Its id is `cmqkjybwo0000ncv4bcjgwgtm` (confirmed against the live database 2026-09-06).
...
> THE CONTROLLED VOCABULARY (never invent terms): single source of truth `packages/db/prisma/collection-vocabulary.ts` — 61 terms across the four tagging axes: OCCASION 32 (christmas, halloween, easter, valentines, diwali, weddings, new-baby...), SEASON 5 (spring/summer/autumn/winter/year-round), STYLE 12 (whimsical, cute, bright-bold, pastel, vintage, cottagecore, modern, boho, scandi, gothic, elegant, geometric), SUBJECT 12 (florals, animals, birds, fantasy-creatures, celestial, landscapes, coastal, food-drink, lettering, people, home-cosy, samplers). Counted 2026-09-06; the file also carries a fifth AUDIENCE axis of 6 terms, so the seeded `CollectionTag` table holds 67 rows in total. Each term has a `taggerGuidance` field — that is the rubric, read it. Pick ONLY existing slugs/aliases. A genuinely missing term is added by editing collection-vocabulary.ts + re-seeding (`pnpm --filter @homemade/db exec tsx scripts/seed-collection-vocabulary.ts`), a deliberate deduped change routed through the orchestrator, NEVER minted inline.
...
Max 4 Sonnet workers concurrent (a credit rule, not a machine cap — see the corrections below); orchestrator tops the pool back to 4 on each completion; unique slug prefixes per worker. Sonnet for bulk generation/authoring; Opus for orchestrator + vision-gate judgement + anchor batches. Weight thin sub-categories over heavy ones; keep subjects fresh (duplication risk rises near target).
...
### Step 6c — How every autopilot runs (Rebecca, 6 September 2026; verified live 2026-09-06)
...
- Working examples in the tree: `docs/autopilot-prompts/cross-stitch.md` (the six-hourly candidate-judging routine, live and firing) and `docs/autopilot-prompts/crochet.md` (the routine exists, schedule created and currently disabled). `apps/web/src/inngest/functions/bulk-generation.ts` carries the cross-stitch and needlework crons; there is deliberately no crochet cron, and `planCrochetBriefs` / `authorCrochetProgram` are refusals that name this rule.
...
(Cross-reference note, 2026-09-06: `[[project_cross_craft_design_direction]]`, `[[project_cross_craft_taxonomy]]` and `[[project_photo_accuracy_solution]]` are not in notes/. Their content lives in the code they name — `packages/db/prisma/design-direction.ts`, `packages/db/prisma/collection-vocabulary.ts` — and, for the photo method, in Step 2 above.)
