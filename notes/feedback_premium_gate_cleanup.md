...
- `lib/studio/premium-gates.ts` `STUDIO_PREMIUM_GATING_ENABLED` is intentionally LEFT FALSE (still `false` in the tree, checked 2026-09-06; `lib/recipes/premium-gates.ts` `RECIPE_PREMIUM_GATING_ENABLED` is still `true`) — it's now the SEWING-only mechanism; do not flip it on cross-stitch's behalf. Cross-stitch enforces premium by calling `hasPremium` directly at the call site (PDF/floss-list/photo-to-chart routes + StudioShell prop), so flipping the shared flag never activates sewing's gates early. New categories should follow the same pattern: enforce with `hasPremium`, don't lean on the shared flag.
...
- The upgrade CTA links to `/premium`. **Corrected 2026-09-06: that page now exists** (`apps/web/src/app/(public)/premium/page.tsx`); the link is no longer dangling. Stripe checkout itself is still Session F.
