/**
 * Pick + export batch 2026-05-28-batch22.
 *
 * Per the cron brief: PUBLISHED + voiceRetrofittedAt IS NULL, take 75 by
 * slug ASC, no category spread, no content-type spread. Then write each
 * tutorial body to docs/voice-retrofit-2026-05-28-batch22/<slug>.json in
 * the shape the voice-check + _pilot-voice-apply scripts expect.
 */
...
async function main() {
  ...
  if (remainingCount === 0) {
    ...
    console.log('[DONE] retrofit complete')
  ...
  console.log(`[exported] ${candidates.length} body files`)
