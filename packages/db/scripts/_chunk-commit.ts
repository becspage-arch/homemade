/** Commit one chunk's verified heroes. Reads chunks/NNN/manifest.json + chunks/NNN/verify.json
 *  (array of {genFile, verdict:"PASS"|"FAIL"}). Commits PASS: R2 upload + VERIFIED flux-pro Media
 *  + hero swap (tx, version snapshot). Idempotent (skips flux-pro / missing). Per-item retry on
 *  transient DB drops. Flag: --chunk N. */
...
async function main() {
  ...
  const base = resolve(RUN, 'chunks', pad)
  ...
  let committed = 0, skipped = 0, notpass = 0, failed = 0, missing = 0
  for (const r of manifest) {
    ...
    for (let attempt = 1; attempt <= 4 && !done; attempt++) {
      try {
        ...
        if (/terminated|ECONNRESET|Closed|timeout|connection|pool/i.test(msg) && attempt < 4) { await prisma.$connect().catch(() => {}); await new Promise((res) => setTimeout(res, 1200 * attempt)); continue }
        failed++; done = true
      }
    }
  }
