...
async function main() {
  ...
  const subCats = await prisma.subCategory.findMany({
    where: { category: { slug: 'sustainability' } },
  ...
  const t = await prisma.tutorial.findFirst({
    where: { slug: 'calculating-loft-insulation-depth' },
    select: { id: true, status: true, publishedAt: true, categoryId: true },
  })
  console.log('calculating-loft-insulation-depth: ' + JSON.stringify(t))
