...
// ── A21 ── Blue Whale ─────────────────────────────────────────────────────────
{
  ...
  const body: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyShape.totalRounds + '. Start with a '), gt('magic-ring', 'magic ring'), t('. Increase to the widest point then '), gt('amigurumi-decrease', 'decrease'), t(' to close each end.')),
...
// ── A22 ── Purple Octopus ─────────────────────────────────────────────────────
{
  ...
  const body: object[] = [
    ...
    h2('Head'),
    p(t('Rounds: ' + headShape.totalRounds + '. Begin with a '), gt('magic-ring', 'magic ring'), t(', then '), gt('amigurumi-increase', 'increase'), t(' each round to the equator, then '), gt('amigurumi-decrease', 'decrease'), t(' to close.')),
...
// ── A23 ── Red Crab ───────────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyShape.totalRounds + '. Start with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' until the equator, then '), gt('amigurumi-decrease', 'decrease'), t(' to close.')),
    ...bodyShape.rowByRow.map(r => p(t('Round ' + r.round + ': ' + r.instructions))),
    h2('Claws (make 2)'),
...
// ── A24 ── Orange Starfish ────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Centre disc'),
    p(t('Rounds: ' + centreShape.totalRounds + '. Start with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' to the equator, then '), gt('amigurumi-decrease', 'decrease'), t('. Stuff lightly before closing.')),
...
// ── A25 ── Grey Dolphin ───────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyPiece.totalRounds + '. Begin with a '), gt('magic-ring', 'magic ring'), t(' at the tail end. '), gt('amigurumi-increase', 'Increase'), t(' to the widest point then '), gt('amigurumi-decrease', 'decrease'), t(' toward the snout end.')),
...
// ── A27 ── Pink Jellyfish ─────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Bell'),
    p(t('Rounds: ' + bellShape.totalRounds + '. Begin with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' to the equator. Begin '), gt('amigurumi-decrease', 'decrease'), t(' rounds then stop halfway to leave the base open; half-stuff before joining the tentacles.')),
...
// ── A28 ── Red Lobster ────────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyPiece.totalRounds + '. Start with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' to the widest point. Work straight, then '), gt('amigurumi-decrease', 'decrease'), t(' to the tail end.')),
    ...bodyPiece.rowByRow.map(r => p(t('Round ' + r.round + ': ' + r.instructions))),
    h2('Claws (make 2)'),
...
// ── A29 ── Green Turtle ───────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyPiece.totalRounds + '. Start with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' to the equator. Begin '), gt('amigurumi-decrease', 'decrease'), t(' rounds but stop early to leave the underside flat for the flippers.')),
    ...bodyPiece.rowByRow.map(r => p(t('Round ' + r.round + ': ' + r.instructions))),
    h2('Shell panel'),
...
// ── A30 ── Grey Shark ─────────────────────────────────────────────────────────
{
  ...
  const bodyNodes: object[] = [
    ...
    h2('Body'),
    p(t('Rounds: ' + bodyPiece.totalRounds + '. Begin at the snout end with a '), gt('magic-ring', 'magic ring'), t(' and '), gt('amigurumi-increase', 'increase'), t(' to the widest point. Work straight, then '), gt('amigurumi-decrease', 'decrease'), t(' toward the tail.')),
    ...bodyPiece.rowByRow.map(r => p(t('Round ' + r.round + ': ' + r.instructions))),
    h2('Dorsal fin'),
