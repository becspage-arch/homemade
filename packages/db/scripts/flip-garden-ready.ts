/**
 * One-off pipeline-status flip for Garden.
 *
 * Sets `Category.pipelineStatus = 'READY'` on the `garden` row so the
 ...
 *
 * Usage:
 *   pnpm --filter "@homemade/db" exec tsx scripts/flip-garden-ready.ts
 *   pnpm --filter "@homemade/db" exec tsx scripts/flip-garden-ready.ts --dry-run
 */
...
async function main(): Promise<void> {
  ...
  if (!before) {
    console.error('[flip] garden category not found. Run seed-categories.ts first.')
    process.exit(2)
  }

  console.log(`[flip] garden: pipelineStatus = ${before.pipelineStatus}`)
  ...
  console.log(`[flip] garden: pipelineStatus = ${after.pipelineStatus} ✔`)
  await prisma.$disconnect()
}
