/**
 * Pick 50 candidates for voice-retrofit batch22.
 * Filter: PUBLISHED + voiceRetrofittedAt IS NULL.
 * Spread: max 15 per category. Batch >= 4 so content-type minimum no longer applies;
 *         keep a soft spread on content-type for variety.
 * Writes docs/voice-retrofit-2026-05-27-batch22/_slugs.json
 */
...
const BATCH_ID = '2026-05-27-batch22'
const TARGET = 50
...
async function main() {
  ...
  const slugs = selected.map((s: any) => s.slug)
