/**
 * Pick the next batch of PUBLISHED tutorials for voice-retrofit batch43.
 *
 * Selection rules per the scheduled-task brief:
 * - voiceRetrofittedAt IS NULL
 * - Status PUBLISHED
 * - Category spread: no more than 21 per category (sustained from batch41-42
 *   where the remaining pool is dominated by a few categories).
 * - Batches 1-3 require content-type spread; batch43 is past that, so
 ...
 *
 * Output: writes docs/voice-retrofit-2026-05-27-batch43/_slugs.json with
...
const BATCH_ID = '2026-05-27-batch43'
const TARGET_COUNT = 63
const MAX_PER_CATEGORY = 21

// Slugs from batch40-42 hand-offs that are blocked by the
