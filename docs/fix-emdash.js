...
const BRIEFS_DIR = path.join(__dirname, 'bulk-batch-019-briefs');
...
for (const [filename, old, replacement] of FIXES) {
  ...
  if (content.includes(old)) {
    ...
    applied++;
    console.log(`  FIXED  ${filename}: ${old.slice(0, 60)}...`);
  } else {
    notFound.push([filename, old.slice(0, 60)]);
    console.log(`  SKIP   ${filename}: not found: ${old.slice(0, 60)}...`);
  }
}
...
if (notFound.length) {
  ...
  for (const [f, s] of notFound) console.log(`  ${f}: ${s}`);
  process.exit(1);
}
